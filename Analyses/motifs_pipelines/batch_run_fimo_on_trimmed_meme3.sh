...
# reference
ref_dir="/mnt/isilon/gandal_lab/mayl/reference/"
...
for meme_file in "$input_dir"/*_trimmed.meme; do
   ...
   nohup  srun --pty fimo --verbosity 2 --max-stored-scores 1000000 --thresh 0.001 --bgfile "$bg_file" --oc "$output_dir" "$meme_file" "$ref_fasta" &
