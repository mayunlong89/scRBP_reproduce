...
# Step 0: 设置参考注释文件路径
# ================================
ref="/mnt/isilon/gandal_lab/mayl/reference/gencode.v44.protein_coding_only_genes.annotation.gtf"
...
for file in *.txt; do
    ...
    final_bed="${base_name}_targets_final.bed"
    ...
    # ------------------------------------------
    bedtools intersect -a "${base_name}.bed" -b "$ref" -wa -wb > "${base_name}_targets.bed"

    # ------------------------------------------
    # Step 7: 提取 gene_name 字段
    # ------------------------------------------
    awk '{print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" gensub(/.*gene_name "([^"]+)".*/, "\\1", "g")}' \
        "${base_name}_targets.bed" > "${base_name}_targets_subset.bed"
    ...
    }' "${base_name}_targets_subset.bed" > "$final_bed"
    ...
    rm -f "${base_name}.score" \
          ...
          "${base_name}.bed" \
          "${base_name}_targets.bed" \
          "${base_name}_targets_subset.bed"
