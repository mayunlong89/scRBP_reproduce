...
##-------Figure 2 ----dot plot-------4 cell types
##-------Figure 2 ----dot plot
...
df <- read.table("Figure_2_monocyte_heatmap_4celltypes.txt", sep = "\t", header = TRUE, check.names = FALSE)
...
df <- df %>%
  mutate(
    Cell_type = factor(Cell_type, levels = c("CD14+ Monocytes","FCGR3A+ Monocytes","CD8+ T cells","CD4+ T cells")),
