...
#2026-02-24
setwd("~/Desktop/UPENN/00UPenn-Projects/02-Aimed_projects/05-devBrain-RBP-methods/00-Manuscript-scRBP-2024/01_Data_analysis/13-scRBP_TRS_test/06_scRBP_trs_Vasculature_6_cardivascular_diseases")
df <- read.csv("01_heatmap_6cardiovascular_diseases_Pvalue_Barplot.csv", check.names = FALSE)

library(dplyr)
library(tidyr)

# phenotype列
pheno_cols <- c("AF_P","CAD_P","DBP_P","HT_P",
                "PP_P","SBP_P")
...
write.csv(sig_ratio,file="01_heatmap_6cardiovascular_diseases_Pvalue_Barplot.csv",quote=F, row.names = F)
...
alpha <- 0.05
pheno_cols <- c("AF_P","CAD_P","DBP_P","HT_P","PP_P","SBP_P")
