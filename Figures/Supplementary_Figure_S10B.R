...
df <- read.csv("01_Summary_lymphocyte_count_results_OpenTargets_SNP.csv",sep="\t" ,check.names = FALSE, stringsAsFactors = FALSE)
df$sample_size <- as.numeric(gsub(",", "", df$sample_size))
df$p_value <- pmax(as.numeric(df$p_value), .Machine$double.xmin)  # 防下溢
df$region    <- factor(df$region, levels = c("5UTR","Introns"))
df$Cell_type <- factor(df$Cell_type, levels = c("T_cells","Monocytes"))
...
p <- ggplot(df1, aes(x = beta, y = y)) +
  ...
  labs(x = "Effect size (Beta)", y = NULL,
       title = "RBP regulons associated with lymphocyte count") +
...
ggsave("rbp_lymphocyte_forest_all22_sorted_by_beta.pdf", p, width = 9, height = h)
p
