...
# 1) Read in your long-format table: Cell_type, RBP, TRS, Dataset, Region
...
# 2) Factor order (modify as needed)
df <- df %>%
  mutate(
...
# 3) —— Robust RBP clustering order: column-wise standardization + NA=0 + Euclidean distance
cluster_rbps_euclid <- function(df) {
  ...
  # Column centering/standardization, avoiding columns with zero variance
  ...
  mat[!is.finite(mat)] <- 0  # NA -> 0 (equivalent to mean after column centering)
...
# If only 1-2 RBPs remain, fall back to sorting by global mean
...
# 4) Plot data: size = |TRS| winsorized; color = positive/negative; fill missing combinations to ensure complete grid
CAP <- 6  # winsorize upper limit (adjustable: 5~8)
dfp <- df %>%
  mutate(
...
rbp_order <- c("DDX21","DDX3X","TRA2A","FAM120A","QKI",
               "YTHDF3","CELF1","YBX3","RBM47","RYBP",
               "AKAP1")
rbp_order_full <- c(rbp_order, setdiff(sort(unique(df$RBP)), rbp_order))
dfp <- dfp %>% mutate(RBP = factor(RBP, levels = rbp_order_full))
# 5) Plot: vertical facets = Cell_type → Region; y-axis within panel = Dataset
p <- ggplot(dfp, aes(x = RBP, y = Dataset)) +
  ...
  scale_fill_manual(values = c(neg = "grey", pos = "#E45756"),
