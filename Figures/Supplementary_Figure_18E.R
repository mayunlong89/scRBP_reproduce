...
# 0) Input / Output
# =========================
infile  <- "01_Summary_9metabolic_traits_results_OpenTargets_SNP.csv"
out_pdf <- "l2g_fill_mlogp_by_disease_ALLSNP_ordered.pdf"

# 你想要的 facet 顺序（固定）
disease_want <- c("HDL","LDL","TC","TG","SHBG","AKP","ALT","TBIL","TST")
...
p <- ggplot(df1, aes(x = mlogp, y = y)) +
  ...
  labs(
    y = NULL,
    title = "Brain disease-associated variants in RBP master regulators are supported by Open Targets"
  ) +
