...
##-------Figure 2 ----dot plot---lymphocyte count and percent
##-------Figure 2 ----dot plot
...
df <- read.table("Figure_2_lymphocyte_count_heatmap.txt", sep = "\t", header = TRUE, check.names = FALSE)
...
df <- df %>%
  mutate(
    Cell_type = factor(Cell_type, levels = c("T_cells_lymp_count","T_cells_lymp_percent","Monocytes_lymp_count","Monocytes_lymp_percent")),
...
rbp_order <- c("EEF2","TAF15","NIPBL","NXF1","PUM1",
               "SRSF1","LARP7","HNRNPC",
               "RTCA","TNRC6C","DDX3X","FMR1","QKI","DEK","RBM47")
