# scRBP reproduce
In this repository, we provided all analytic and visualized codes and examples to facilite users to understand [scRBP](https://github.com/mayunlong89/scRBP) method and reproduce our results.

<table>
  <tr>
    <td>
      <img src="https://github.com/mayunlong89/scRBP/blob/main/Examples/scRBP_regulon_logo.png" width="350">
    </td>
    <td>
      <h1>scRBP</h1>
      <p>A scalable framework for inferring RNA-binding protein regulons from single-cell data</p>
    </td>
  </tr>
</table>


![pypi](https://img.shields.io/badge/pypi-0.1.2-green)
![python](https://img.shields.io/badge/python-3.9--3.11-blue)
![license](https://img.shields.io/badge/license-MIT-yellow)

[scRBP](https://github.com/mayunlong89/scRBP) (single-cell RNA Binding Protein regulon inference) is a computational framework that reconstructs RNA-binding protein (RBP)-mediated regulatory networks (regulons) from single-cell transcriptomes, integrating motif-binding evidence and polygenic signals from genome-wide association studies (GWAS) to prioritize trait-associated RBP programs across diverse cellular contexts.


## More information
For more details, please refer to [scRBP protocols](https://mayunlong89.github.io/scRBP.github.io/), or [scRBP description](https://pypi.org/project/scRBP), or [scRBP.github.io](https://github.com/mayunlong89/scRBP.github.io).

## scRBP motif collection
Please refer to the [Gandallab resource](https://resources.gandallab.org/motif-collection.html), we provided 616 RBPs and their corresponding 20746 motifs with quality metrics in this [resource](https://resources.gandallab.org/motif-collection.html). 


## Citation

If you use scRBP in your research, please cite:

> Ma Y. *et al.* ***Single-cell maps of RNA-binding protein networks reveal post-transcriptional architecture across development and disease***. (2026)

---
